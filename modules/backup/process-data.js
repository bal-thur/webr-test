...
function createSelectControl({
    ...
    wrapper.className = "process-control";
...
function populateMultiSelect(choices, options) {
...
function getProcessingParameters(controls) {
...
function emptyArrayToNull(value) {
...
async function getProcessingOptions(webR) {
...
async function runProcessing(webR, parameters) {

    const runProcessSE =
        await webR.evalR("run_process_se");


    // Ejecutar process_se() en R
    await runProcessSE(
      emptyArrayToNull(parameters.remove_groups),
      emptyArrayToNull(parameters.remove_subgroups),
      emptyArrayToNull(parameters.remove_samples),
...
function addProcessInterface() {
    ...
    const featureFilter = createSelectControl({
        labelText: "Feature filtering",
        id: "process-feature-filter"
    });


    const imputation = createSelectControl({
        labelText: "Imputation",
        id: "process-imputation"
    });


    const normalization = createSelectControl({
        labelText: "Normalization",
        id: "process-normalization"
    });


    const transform = createSelectControl({
    ...
    const groups = createMultiSelectControl({
    ...
    const subgroups = createMultiSelectControl({
    ...
    const samples = createMultiSelectControl({
    ...
    processButton.id = "process-data-button";
  ...
  //  document.body.appendChild(container);
    ...
    return {
        ...
        featureFilter: featureFilter.select,

        imputation: imputation.select,

        normalization: normalization.select,

        transform: transform.select,

        groups: groups.choices,

        subgroups: subgroups.choices,
...
export async function initProcessDataModule({
    ...
    log("Inicializando módulo de procesamiento...");


    // --------------------------------------------------------
    // Crear interfaz

    const controls = addProcessInterface();
    
       // --------------------------------------------------------
    // Promise de procesamiento
...
//    const processCompleted = new Promise(
//      (resolve, reject) => {
...
//      );
    ...
    try {
        ...
        options = await getProcessingOptions(webR);
...
controls.processButton.addEventListener(
    ...
    async () => {
        ...
        const parameters =
            getProcessingParameters(controls);
        ...
        try {
            ...
            controls.processButton.disabled = true;
            ...
            // Resolver la Promise.
            //
            // Los futuros módulos podrán esperar a que
            // termine el procesamiento.
            if (onProcessCompleted) { 
              onProcessCompleted(summary);
            ...
            controls.processButton.disabled = false;
        }
    }
);
    ...
    log("Módulo de procesamiento listo.");
