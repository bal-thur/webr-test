...
${indentData}`);
        ...
        concat(other, by = "row") {
          ...
          switch (by) {
            case "row": {
              ...
              const result = new Matrix3(this.rows + other.rows, this.columns);
            ...
            case "column": {
              ...
              const result = new Matrix3(this.rows, this.columns + other.columns);
              result.setSubMatrix(this, 0, 0);
      ...
      function linearDependencies2(matrix2, options = {}) {
        ...
        for (let i = 0; i < n; i++) {
          ...
          let x = svd.solve(b);
          let scale = Matrix3.abs(b).max() || 1;
          let error = Matrix3.sub(b, Abis.mmul(x)).abs().max() / scale;
          results.setRow(
            i,
  ...
  // hclust-entry.js
  var hclust_entry_exports = {};
  __export(hclust_entry_exports, {
    agnes: () => agnes
  });
  ...
  // node_modules/ml-distance-matrix/src/index.js
  ...
  // node_modules/ml-hclust/lib/cluster.js
  ...
  var Cluster = class _Cluster {
    ...
    group(groups) {
      ...
      const heap = new import_heap.default((a, b) => {
  ...
  // node_modules/ml-hclust/lib/agnes.js
  ...
  function agnes(data, options = {}) {
    ...
    let distanceData;
    if (!isDistanceMatrix) {
      distanceData = distanceMatrix(data, distanceFunction);
    } else {
      distanceData = data;
    }
    let distanceMatrix2 = new Matrix2(distanceData);
    const numLeaves = distanceMatrix2.rows;
    ...
    for (let n = 0; n < numLeaves - 1; n++) {
      const [row, column, distance] = getSmallestDistance(distanceMatrix2);
      ...
      const newDistanceMatrix = new Matrix2(distanceMatrix2.rows - 1, distanceMatrix2.rows - 1);
      for (let i = 1; i < newDistanceMatrix.rows; i++) {
        ...
        for (let j = 0; j < i; j++) {
          if (j === 0) {
            const dKI = distanceMatrix2.get(row, prevI);
            const dKJ = distanceMatrix2.get(prevI, column);
            ...
            const val = distanceMatrix2.get(prevI, previous(j, row, column));
            newDistanceMatrix.set(i, j, val);
            newDistanceMatrix.set(j, i, val);
          }
        }
      ...
      distanceMatrix2 = newDistanceMatrix;
    }
    return clusters[0];
  }
  ...
  return __toCommonJS(hclust_entry_exports);
})();
